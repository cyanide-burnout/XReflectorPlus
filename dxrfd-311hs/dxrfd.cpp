...
static void send_heartbeat()
{
   ...
   for (pos = inbound_list.begin(); pos != inbound_list.end(); pos++)
   {
      ...
      // R3ABM patch
      if ((block_pos == blocks.end()) && (controller.validateCallSign(inbound_ptr->call)))  /* not blocked */
...
{
   short int valid_params = 10 /* R3ABM patch begin */ + CONTROLLER_PARAMETERS_COUNT /* R3ABM patch end */;
   short int params = 0;
   ...
   while (fgets(inbuf, 1020, cnf) != NULL)
   {
      ...
      *p = '\0';
      ...
      if (controller.configure(inbuf, p + 1))
        params ++;
      /* R3ABM patch end */
   }
   fclose(cnf);
...
static void check_heartbeat()
{
   ...
   for (pos = a_user_list.begin(); pos != a_user_list.end(); pos++) 
   {
      ...
      // R3ABM patch
      if ((block_pos == blocks.end()) && (controller.validateCallSign(a_user_ptr->call)))  /* not blocked */
      // if (block_pos == blocks.end())  /* not blocked */
      {
             ...
             (refbuf[3] == 0))
         {
            ...
            ***/
            ...
            if ((call_valid != REG_NOERROR) ||
                ...
                /* R3ABM patch begin */ (!controller.validateConnection(a_call2, an_ip)) || /* R3ABM patch end */
            ...
            if (inbound_pos != inbound_list.end())
            {
               ...
               if (recvlen3 == 58)
               {
                  ...
                  else
                  {
                     a_call[7] = ' ';
                     // R3ABM patch
                     if ((blocks.find(a_call) != blocks.end()) || (!controller.validateCallSign(a_call)))
...
// R3ABM patch end
            ...
            else
            {
               ...
               do 
               {
                  ...
                  // R3ABM patch
                  if ((block_pos != blocks.end()) || (!controller.validateCallSign(a_call)))
...
// R3ABM patch begin
                  if (!controller.validateConnection(a_call, an_ip))
                  {
                    allowed_to_connect = false;
                    break;
                  }
...
// R3ABM patch begin
                     if (controller.getAccessType(a_call) == 0)
// R3ABM patch end
